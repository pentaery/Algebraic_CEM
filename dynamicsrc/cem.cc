...
CEM::CEM()
    : matA(nullptr), matL(nullptr), matR(nullptr), matM(nullptr),
...
void CEM::getDatafromMFEM(const char *mesh_file, int order) {
  ...
  // Assemble mass matrix M via MFEM and store as CSR.
  {
    mfem::BilinearForm m(&fespace);
    m.AddDomainIntegrator(new mfem::MassIntegrator);
    m.Assemble();

    mfem::SparseMatrix M_sparse;
    m.FormSystemMatrix(ess_tdof_list, M_sparse);
    M_sparse.SortColumnIndices();

    const int m_nnz = M_sparse.NumNonZeroElems();
    const int *MI = M_sparse.GetI();
    const int *MJ = M_sparse.GetJ();
    const double *MData = M_sparse.GetData();

    matM_rows_start.assign(static_cast<size_t>(nvtxs), 0);
    matM_rows_end.assign(static_cast<size_t>(nvtxs), 0);
    matM_col_index.assign(static_cast<size_t>(m_nnz), 0);
    matM_values.assign(static_cast<size_t>(m_nnz), 0.0);

    for (MKL_INT row = 0; row < nvtxs; ++row) {
      const int row_i = static_cast<int>(row);
      matM_rows_start[static_cast<size_t>(row)] =
          static_cast<MKL_INT>(MI[row_i]);
      matM_rows_end[static_cast<size_t>(row)] =
          static_cast<MKL_INT>(MI[row_i + 1]);
    }
    for (int k = 0; k < m_nnz; ++k) {
      matM_col_index[static_cast<size_t>(k)] = static_cast<MKL_INT>(MJ[k]);
      matM_values[static_cast<size_t>(k)] = MData[k];
    }

    destroyIfAllocated(matM);
    const sparse_status_t m_status = mkl_sparse_d_create_csr(
        &matM, indexing, nvtxs, nvtxs, matM_rows_start.data(),
        matM_rows_end.data(), matM_col_index.data(), matM_values.data());
    if (m_status != SPARSE_STATUS_SUCCESS) {
      throw std::runtime_error(
          "mkl_sparse_d_create_csr(matM) failed with status " +
...
void CEM::solveFromLAndReleaseA(double deltaT, int n) {
  auto start = std::chrono::high_resolution_clock::now();

  if (matL == nullptr || matM == nullptr) {
    throw std::runtime_error(
        "matL or matM is not initialized. Call getDatafromMFEM() first.");
  }
  if (nvtxs <= 0 || deltaT <= 0.0 || n <= 0) {
    throw std::runtime_error("Invalid parameters for forward Euler solve.");
  }

  destroyIfAllocated(matA);

  // ---- Export L and build upper-triangular A for Au products ----
  MKL_INT *l_rows_start = nullptr, *l_rows_end = nullptr;
  MKL_INT *l_col_index = nullptr;
  double *l_val = nullptr;
  ...
  if (status != SPARSE_STATUS_SUCCESS) {
    throw std::runtime_error("mkl_sparse_d_export_csr(matL) failed.");
  }

  // Upper-triangular A: A(i,i)=sum_j L(i,j), A(i,j)=-L(i,j) for j>i.
  std::vector<MKL_INT> ia_A(static_cast<size_t>(nvtxs) + 1, 0);
  for (MKL_INT i = 0; i < nvtxs; ++i) {
    MKL_INT cnt = 1; // diagonal
    for (MKL_INT j = l_rows_start[i]; j < l_rows_end[i]; ++j)
      if (l_col_index[j] > i)
        ++cnt;
    ia_A[static_cast<size_t>(i + 1)] = ia_A[static_cast<size_t>(i)] + cnt;
  }
  const MKL_INT nnz_A = ia_A[static_cast<size_t>(nvtxs)];
  std::vector<MKL_INT> ja_A(static_cast<size_t>(nnz_A), 0);
  std::vector<double> a_A(static_cast<size_t>(nnz_A), 0.0);
  for (MKL_INT i = 0; i < nvtxs; ++i) {
    MKL_INT w = ia_A[static_cast<size_t>(i)];
    ja_A[static_cast<size_t>(w)] = i;
    double diag = 0.0;
    for (MKL_INT j = l_rows_start[i]; j < l_rows_end[i]; ++j)
      diag += l_val[j];
    a_A[static_cast<size_t>(w)] = diag;
    ++w;
    for (MKL_INT j = l_rows_start[i]; j < l_rows_end[i]; ++j) {
      if (l_col_index[j] > i) {
        ja_A[static_cast<size_t>(w)] = l_col_index[j];
        a_A[static_cast<size_t>(w)] = -l_val[j];
        ++w;
      }
    }
  }

  // ---- Export M and build upper-triangular M for PARDISO ----
  MKL_INT *m_rows_start = nullptr, *m_rows_end = nullptr;
  MKL_INT *m_col_index = nullptr;
  double *m_val = nullptr;
  status = mkl_sparse_d_export_csr(matM, &indexing, &rows, &cols, &m_rows_start,
                                   &m_rows_end, &m_col_index, &m_val);
  if (status != SPARSE_STATUS_SUCCESS) {
    throw std::runtime_error("mkl_sparse_d_export_csr(matM) failed.");
  }

  std::vector<MKL_INT> ia_M(static_cast<size_t>(nvtxs) + 1, 0);
  for (MKL_INT i = 0; i < nvtxs; ++i) {
    MKL_INT cnt = 0;
    for (MKL_INT j = m_rows_start[i]; j < m_rows_end[i]; ++j)
      if (m_col_index[j] >= i)
        ++cnt;
    ia_M[static_cast<size_t>(i + 1)] = ia_M[static_cast<size_t>(i)] + cnt;
  }
  const MKL_INT nnz_M = ia_M[static_cast<size_t>(nvtxs)];
  std::vector<MKL_INT> ja_M(static_cast<size_t>(nnz_M), 0);
  std::vector<double> a_M(static_cast<size_t>(nnz_M), 0.0);
  for (MKL_INT i = 0; i < nvtxs; ++i) {
    MKL_INT w = ia_M[static_cast<size_t>(i)];
    for (MKL_INT j = m_rows_start[i]; j < m_rows_end[i]; ++j) {
      if (m_col_index[j] >= i) {
        ja_M[static_cast<size_t>(w)] = m_col_index[j];
        a_M[static_cast<size_t>(w)] = m_val[j];
        ++w;
      }
    }
  }
  ...
  MKL_INT error = 0;
  MKL_INT maxfct = 1, mnum = 1, mtype = 2; // SPD
