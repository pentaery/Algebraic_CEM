...
public:
  ...
  void solveFromLAndReleaseA(double deltaT = 1e-2, int n = 100);
...
private:
  ...
  // Backing storage for MKL CSR handle for mass matrix M from MFEM data.
  std::vector<MKL_INT> matM_rows_start;
  std::vector<MKL_INT> matM_rows_end;
  std::vector<MKL_INT> matM_col_index;
  std::vector<double> matM_values;
  ...
  // cemBasis[i] stores k0 basis vectors over overlap vertices in part i,
  // flattened as [k][local_vertex].
